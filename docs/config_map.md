...
| Origen | Qué es |
|--------|--------|
| `.env` / [app/config.py](app/config.py) | `DISCORD_TOKEN`, OAuth client id/secret/redirect, `SESSION_SECRET`, `DATABASE_PATH`, `APP_BASE_URL`, límites por defecto (`DEFAULT_*`). No editables desde el panel por seguridad. |
